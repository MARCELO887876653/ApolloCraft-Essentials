...
/**
 * Recognizer for our own built-in chunk generators, addressed canonically as {@code ApolloCraftEssentials:<id>}. Only
...
public final class BuiltInGenerators {
    ...
    private static final String NAMESPACE = "apollocraftessentials:";
    private static final String LEGACY_NAMESPACE = "uxmessentials:";
    ...
    public static Optional<String> idOf(String generatorRef) {
        ...
        if (lower.startsWith(NAMESPACE)) {
            id = lower.substring(NAMESPACE.length());
        } else if (lower.startsWith(LEGACY_NAMESPACE)) {
            id = lower.substring(LEGACY_NAMESPACE.length());
        } else {
    ...
    /** The canonical namespaced ref for a built-in id, e.g. {@code ApolloCraftEssentials:void}. */
    public static GeneratorRef ref(String id) {
        Objects.requireNonNull(id, "id");
        return GeneratorRef.of("ApolloCraftEssentials:" + id);
    }
}
