...
Versão personalizada do **ApolloCraft**, baseada no projeto open-source **uxmEssentials**. Não é uma release oficial da UXPLIMA.
Todos os módulos originais permanecem no código. O ApolloCraft adiciona uma central administrativa para ativar/desativar módulos sem removê-los do `.jar`; mudanças de estado são aplicadas no próximo reinício do servidor.

- Comando principal: `/apollo`
- Menu de módulos: `/apollo modules`
- Aliases legados preservados: `/uxmess`, `/uxmessentials`, `/uxe`
- Idioma padrão: Português
- Licença e créditos do projeto original preservados em `LICENSE` e `NOTICE-APOLLOCRAFT.md`.
...
It is built **Paper 26.2 only, on purpose**: the current API used the way it is meant to be used,
...
| | |
| --- | --- |
| Server | Paper **26.2** (build 112), or a Paper fork (Folia / Purpur / Pufferfish) |
| Java | **25** |
| Optional | PlaceholderAPI · Vault / VaultUnlocked / Treasury · a Votifier-compatible vote listener · LuckPerms · WorldGuard |
| Clients | any version a server-side ViaVersion supports can connect; the **server** must be 26.2 |
...
- **Tests**: JUnit 5, AssertJ, Mockito, MockBukkit (Paper 26.2), Testcontainers, and jqwik property
  tests.
